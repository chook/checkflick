...
public class CheckFlickGUI {
	...
	public static void ShowPersonResult(RibbonTab tab,final PersonEntity person){
		...
		save.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				String newRName = person.getPersonRealName();
				String newNName = person.getPersonNickNames();
				int newBYear = person.getYearOfBirth(); 
				int newDYear = person.getYearOfDeath();
				int newCountry = person.getCountryOfBirth();
				int newHeight = person.getHeight();
				String newCity = person.getCityOfBirth();
				String newBio = person.getBiography();
				String newTM = person.getTrademark();
				boolean valid = true;
				boolean update = false;
				if (newName.compareTo(nameText.getText()) != 0){
					if (nameText.getText()!=""){
				...
				if (update && valid){
					AbsType updatedPerson = new PersonEntity(person.getId(), newName ,newRName , newNName , person.getDateOfBirth() , newBYear ,newCity, newCountry, person.getDateOfDeath() , newDYear ,newHeight, newTM , newBio );
					try {
						pool.execute(DataManager.updatePersonData(PersonDataEnum.PERSON, updatedPerson));
					} catch (InterruptedException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
	...
	static protected void drawUpdateDataSuccess() {
		display.asyncExec(new Runnable() {
			public void run() {
